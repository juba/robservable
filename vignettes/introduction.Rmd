...
Note that it is not just an `iframe` embedding a whole notebook.  Cells are integrated directly into the document or application with no `iframe` wraper.  You can choose what cells to display, update cell values from R, and add observers to cells to get their values back into a Shiny application.
