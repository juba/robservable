...
By default, `robservable` overrides the potential `width` and `height` notebook values by the htmlwidget root HTML element width and height. This override is performed both at widget creation and at widget resizing. Overriding `width` allows to make the widget "fit" on its container, and avoids it to change size when the page is resized but not its container (which is the case when `width` is taken from Observable standard library). Overriding `height` has the same purpose, but as all notebooks don't define an `height` value, it won't always have an effect.
...
## Advices on writing notebooks for robservable

If you are developping Observable notebooks to be used with robservable, here are some advices to make your notebook more easy to use in R :

- one way to see a notebook is as a function. Most cells are input arguments, and others are output with results.
- to allow the output to be customized in R, move as much settings and values as possile in their own cell.
- name all the cells that could be included or whose value could be modified in R.
- give a name to the CSS `<style>` cells, this will ease their inclusion in R.
- making your chart updatable with animated transitions can be a nice addition if the notebook could be used in a shiny app.
pkgdown
