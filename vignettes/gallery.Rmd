...
Then we can call `robservable` to load the notebook, render only `chart` and `draw` (both are needed for the map to show up), hide `draw` and update a bunch of cells avalues via the `input` named list. You can refer to the notebook for an explanation of the different values.
